...
export default function DisplayRecipe({ recipe, isClicked, handleClose }) {
  ...
  const updateInfo = (t_id) => {
    fetch("http://localhost:8000/api/users/" + recipe.createdByID)
      .then((res) => res.json())
      .then((json) => {
        setUserName(json.name);
      });

    fetch("http://localhost:8000/api/user-recipes/" + t_id)
      .then((res) => res.json())
      .then((json) => {
        if (json.length > 0) {
      ...
      {recipe != null && createdByUser != null ? (
        <div className="Display_Recipe_Container">
          ...
          <div className="row">
            <div
              style={{ padding: 0 }}
              class="col-sm d-flex justify-content-end"
            >
              <Typography className="Display_Recipe_Text">
                Average Rating:
              </Typography>
            ...
            <div
              style={{ padding: 0 }}
              class="col-sm d-flex justify-content-start"
            >
              {avgRating !== null ? (
                <Rating
                  value={avgRating}
          ...
          {LoggedInid === null || LoggedInid === "null" ? (
            <Typography
              ...
              className="Display_Recipe_Text"
            >
              Login to leave a review or save this recipe to your collection
            </Typography>
          ) : (
            <>
              <Button
